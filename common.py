...
######################################改进###################################################################################
#########yolov7的ELAN改进####################################################################################################
class ELAN_DBB(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(ELAN_DBB, self).__init__()
        ...
        self.conv6 = Conv(hidc, hidc, k=3, act=act)
        self.conv7 = Conv(hidc * 4, ouc, k=1, act=act)
        self.dbb = DiverseBranchBlock(hidc, hidc, k=3, act=act)
...
#########yolov7-tiny的MCB改进##################################################################################################  
class MCB_DBB(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(MCB_DBB, self).__init__()
...
#########VoVGSCSP-DCN-CBAM##################################################################################################################
class VoVGSCSP_DCN_CBAM(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        ...
        self.gsb = nn.Sequential(*(GSBottleneck(c_, c_, e=1.0) for _ in range(n)))
        self.cv3 = Conv(2 * c_, c2, 1) 
        self.dcn = DCNv2(c_, c_, 3, 1, act=False)
        self.att = CBAMBlock(c2)

    def forward(self, x):
        x1 = self.dcn(self.gsb(self.cv1(x)))
        y = self.cv2(x)
        return self.att(self.cv3(torch.cat((y, x1), dim=1)))
    
    
#########VoVGSCSP-DCN############################################################    
class VoVGSCSP_DCN(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
...
class VoVGSCSPDCN_CBAM(nn.Module):
    # VoVGSCSP module with GSBottleneck
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.gsb = nn.Sequential(*(GSBottleneck(c_, c_, e=1.0) for _ in range(n)))
        self.res = DCNv2(c_, c_, 3, 1, act=False)
        #self.res = Conv(c_, c_, 3, 1, act=False)
        self.cv3 = Conv(2 * c_, c2, 1)  #
        self.att = CBAMBlock(c2)

    def forward(self, x):
        x1 = self.res(self.gsb(self.cv1(x)))
        y = self.cv2(x)
        return self.att(self.cv3(torch.cat((y, x1), dim=1)))
    

    
class Yolov7_ELANDBB(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_ELANDBB, self).__init__()
